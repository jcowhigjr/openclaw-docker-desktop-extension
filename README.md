...
Current status as of 2026-09-22: the MVP is complete enough to share. The latest release is `v0.5.0`, shipped via the new gated release-please automation (#229/#230/#232) rather than hand-tagging. GHCR `stable` resolves to `v0.5.0` on both the extension and runtime images, publicly readable. Docker's own Marketplace validation job passed on `v0.5.0` on 2026-09-21 (#86), with no known repo-side blocker remaining — but Docker has separately said Marketplace publication of new extensions is paused platform-wide with no timeline given, so validation passing does not mean publication is close. Two material gaps found during a hands-on review pass are not yet fixed: recreating the service container has no safe-recreate path and can brick a working install (#215, confirmed once in the wild), and no host folder can be mounted, so the agent cannot act on any file on the user's Mac (#216). #234 fixed the false-positive downgrade banner behind #215's trigger, but that fix is on `main` only — it is not in the `v0.5.0` tag users install today, since v0.5.0 was tagged before #234 merged. See [docs/submission-readiness.md](docs/submission-readiness.md) for the full current-limits list before relying on this for anything beyond a trial. The latest committed real Docker Desktop stable-channel UI smoke packet remains `v0.3.4` in [docs/exploratory/2026-05-22-stable-channel-smoke/](docs/exploratory/2026-05-22-stable-channel-smoke/); a CLI/registry-only `v0.5.0` pass exists in [docs/exploratory/2026-09-22-stable-channel-smoke/](docs/exploratory/2026-09-22-stable-channel-smoke/) but did not include the manual UI walkthrough.
