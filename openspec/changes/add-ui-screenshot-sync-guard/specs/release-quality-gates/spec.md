...
- **WHEN** the evaluated commit range contains changes to files under `ui/src/` and at least one image file under `docs/` also changed in that range
...
The guard SHALL provide an explicit, auditable opt-out so that behavior-only UI changes (no visual difference) are not blocked. The opt-out MUST be expressed in commit metadata within the evaluated range using a trailer of the form `Screenshots-Not-Needed: <reason>`, and the reason MUST be non-empty.
...
- **AND** a commit in the range contains a `Screenshots-Not-Needed:` trailer with a non-empty reason
...
To keep screenshots fresh, demo mode SHALL be able to render each first-run onboarding phase deterministically via a query flag, so the capture pipeline can screenshot new onboarding screens without live first-run state.

#### Scenario: Demo flag forces the fork onboarding phase

- **WHEN** the UI loads with the demo onboarding flag set to the fork phase (e.g. `?demo=1&onboarding=fork`)
- **THEN** the UI SHALL present first-run inputs (unset provider, no configured model, empty model list) such that the fork onboarding card renders
...
- **THEN** it SHALL exercise the failing case, the screenshot-updated passing case, and the valid opt-out case using fixtures or a temporary git repository, independent of the project's real commit history
